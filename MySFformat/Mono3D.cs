...
namespace MySFformat
{
    ...
    class Mono3D : Game
    {
        ...
        private void deleteMeshVertexFaceset(SoulsFormats.FLVER2.Mesh m, uint index)
        {
            foreach (var fs in m.FaceSets)
            {
                for (int i = 0; i + 2 < fs.Indices.Count; i += 3)
                {
                    ...
                    {
                        fs.Indices[i] = (int)index;
                        fs.Indices[i + 1] = (int)index;
                        fs.Indices[i + 2] = (int)index;
                    }
                }
            }
        }
        ...
        protected void editVerticesInfo()
        {
            ...
            if (targetV != null)
            {
                ...
                string positions = Program.FormatOutput(new System.Web.Script.Serialization.JavaScriptSerializer().Serialize(targetV.Position));
                TextBox tbp = new TextBox();
                ...
                bn.Text = "Modify";
                bn.Click += (s, o) => {
                    System.Numerics.Vector3 vp = new System.Web.Script.Serialization.JavaScriptSerializer().Deserialize<System.Numerics.Vector3>(tbp.Text);
        ...
        protected void checkVertices()
        {
            ...
            if (targetV.Normal != null && !targetV.Normal.Equals(0))
            {
                Program.checkingPointNormal = new System.Numerics.Vector3(targetV.Normal.X, targetV.Normal.Y, targetV.Normal.Z);
            }
            else
            ...
            if (mState.LeftButton == Microsoft.Xna.Framework.Input.ButtonState.Pressed && !Keyboard.GetState().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.LeftAlt) && IsActive)
            {
                ...
                foreach (SoulsFormats.FLVER.Vertex v in Program.vertices) {
                    ...
                    if (ptDistance >= dis) {
                        //TODO: smarter painting
                        //Need some way to not paint the verts obscured by other faces
                        if (v.Normal != null && !v.Normal.Equals(0)) {
                            //Only paint front faces to help avoid bleedthrough
                            Vector3 normal = new Vector3(v.Normal.X, v.Normal.Y, v.Normal.Z);
